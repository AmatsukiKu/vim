" Language:	   Colored SASS Color Preview
" Maintainer:	Niklas Hofer <niklas+vim@lanpartei.de>
" URL:		   svn://lanpartei.de/vimrc/after/syntax/sass.vim
...
function! s:PreviewSASSColorInLine(where)
   " TODO use sassColor matchdata
...
if has("gui_running") || &t_Co==256
   " HACK modify sassDefinition to add @sassColors to its contains
   redir => s:olddef
      silent!  syn list sassDefinition
   redir END
   if s:olddef != ''
      let s:b = strridx(s:olddef,'matchgroup')
      if s:b != -1
         exe 'syn region sassDefinition '.strpart(s:olddef,s:b).',@sassColors'
      endif
   endif
   ...
   let i = 1
   while i <= line("$")
      call s:PreviewSASSColorInLine(i)
      let i = i+1
   endwhile
   unlet i

   autocmd CursorHold * silent call s:PreviewSASSColorInLine('.')
   autocmd CursorHoldI * silent call s:PreviewSASSColorInLine('.')
   set ut=100
